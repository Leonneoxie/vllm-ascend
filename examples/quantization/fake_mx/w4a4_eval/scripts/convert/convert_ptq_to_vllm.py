...
def convert_rht(ptq_dir, output_path):
    """Convert RHT params to a single-key safetensors.

    RHT uses a fixed Rademacher sign diagonal (seed=0) that is identical for
    all layers. vllm-ascend's _copy_rht_signs checks for a global "rht_signs"
    key first, so we only need to save one tensor (int8, ±1).
    """
    pt_files = []
    for root, _dirs, files in os.walk(ptq_dir):
        for f in files:
            if f.endswith(".pt"):
                pt_files.append(os.path.join(root, f))
    if not pt_files:
        raise FileNotFoundError(f"No .pt files found in {ptq_dir}")

    # All RHT signs are identical (seed=0); load the first one
    d = torch.load(pt_files[0], map_location="cpu")
    rht_signs = d["input_transform"]["rht_signs"]

    # Validate int8 ±1
    if rht_signs.dtype != torch.int8:
        raise ValueError(f"rht_signs must be int8, got {rht_signs.dtype}")
    if not torch.all((rht_signs == -1) | (rht_signs == 1)):
        raise ValueError("rht_signs must contain only -1 and +1")

    save_file({"rht_signs": rht_signs.contiguous()}, output_path)
    print(f"Saved RHT signs (shape={tuple(rht_signs.shape)}, int8 ±1) to {output_path}")
    print(f"  Source: {pt_files[0]} ({len(pt_files)} total .pt files, all identical)")
...
def main():
    ...
    parser.add_argument(
        "--algo",
        choices=["flatquant", "lht", "rht", "omniquant", "autoround", "lwc", "lac"],
        help="Algorithm name",
    )
    ...
    if args.algo == "flatquant":
        ...
        convert_rht(args.ptq_dir, args.output)
