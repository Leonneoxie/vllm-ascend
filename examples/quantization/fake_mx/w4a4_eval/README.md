...
└── scripts/
    ...
    ├── eval/run_mmlu_pro.py           # MMLU-Pro 评测
    ├── eval/run_livecodebench.py      # LiveCodeBench 评测
    ├── eval/run_three_datasets.sh     # 三数据集串行入口
    ├── eval/run_level3_verify.py      # Level 3 快速验证（105 题，~10min）
    ├── eval/run_limited_eval.py       # 小样本快速验证（limit per-subset）
    ├── ptq/extract_ptq_data.sh        # 校准数据提取
    ├── ptq/run_amct_ptq.sh            # PTQ 多卡训练
...
五个算法的变换数学与参数 sidecar 契约统一见 [fake_mx README](../README.md) 的 "Algorithm checkpoint contracts"；各配置 JSON key 的含义与代码读取位置见本文档末尾的[配置项与代码对照表](#配置项与代码对照表)。

### RTN（Round-To-Nearest）

直接对权重和激活做 MXFP4 QDQ，无任何变换。无需 PTQ 训练，无需外部参数。

- scheme 名：`W4A4_MXFP4_FAKE` / `W8A8_MXFP8_FAKE`
- 代码位置：`fake_mx_algorithms/linear.py`

### RHT（Randomized Hadamard Transform）

量化前对权重和激活施加随机 Hadamard 变换，打散离群值。运行时用 `rht_seed`（默认 0，与 AMCT 一致）确定性生成 signs，无需 PTQ 训练；可选 `rht_params_path` 加载 AMCT 导出的 `rht_signs` 并逐层比特级校验。

- scheme 名：`W4A4_MXFP4_RHT_FAKE` / `W8A8_MXFP8_RHT_FAKE`
- 代码位置：`fake_mx_algorithms/rht.py`

### FlatQuant

学习型 Kronecker 变换，PTQ 训练优化 left/right 变换矩阵和 diag_scale。
...
学习型逐维缩放，PTQ 训练优化 `log_scale`（权重乘 `exp(log_scale)`、激活除，夹在 [1e-4, 1e4]）。

- scheme 名：`W4A4_MXFP4_OMNIQUANT_FAKE` / `W8A8_MXFP8_OMNIQUANT_FAKE`
- 代码位置：`fake_mx_algorithms/omniquant.py`
- 参数文件格式（safetensors）：`{prefix}.log_scale` — 逐输入维缩放 [1, in_features]

### LHT（Learnable Hadamard Transform）

学习型 Hadamard 变换，PTQ 训练优化 K×K 变换矩阵（`lht_params_path` 必填）。

- scheme 名：`W4A4_MXFP4_HADAMARD_LEARNING_FAKE` / `W8A8_MXFP8_HADAMARD_LEARNING_FAKE`
- 代码位置：`fake_mx_algorithms/lht.py`
- 参数文件格式（safetensors）：`{prefix}.transform_weight` — K×K 变换矩阵 [K, K] (bfloat16)；K = `hadamard_learning_matrix_size`（默认 128），in_features 必须能被 K 整除
