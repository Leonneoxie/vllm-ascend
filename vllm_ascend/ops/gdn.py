...
class AscendGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def forward(
        ...
        # The projection method quantizes the Linear operands, but its output
        # feeds a separate recurrent attention operator. Insert the A4 fake-MX
        # boundary before the core splits into prefill/decode implementations.
        qkv_projection = self.in_proj_qkv if hasattr(self, "in_proj_qkv") else self.in_proj_qkvz
        (mixed_qkv,) = maybe_fake_mx_quantize_activations(qkv_projection, mixed_qkv)
