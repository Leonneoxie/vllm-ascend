...
def _copy_rht_signs(
    ...
    if "rht_signs" in params:
        key, value = "rht_signs", params["rht_signs"]
    else:
        match = _find_transform_param(params, layer, "rht_signs")
        if match is None:
            prefix = getattr(layer, "prefix", "") or "<unknown>"
            raise KeyError(f"Missing 'rht_signs' transform parameter for layer {prefix!r}.")
        key, value = match
    if value.dtype != torch.int8 or target.shape != value.shape:
        raise ValueError(
            f"RHT parameter {key!r} must be int8{tuple(target.shape)}, "
            f"got {value.dtype}{tuple(value.shape)}."
        )
    if not torch.all((value == -1) | (value == 1)):
        raise ValueError(f"RHT parameter {key!r} must contain only -1 and +1.")
...
class _AscendFakeMXLinearMethod(AscendLinearScheme):
    ...
    def __init__(self):
        ...
        targets = quant_description.get("fake_mx_quant_targets", ["attn-linear"])
        if not isinstance(targets, list) or not all(isinstance(target, str) for target in targets):
            raise TypeError("fake_mx_quant_targets must be a list of strings.")
        unknown_targets = set(targets) - SUPPORTED_QUANT_TARGETS
        if unknown_targets:
            raise ValueError(
                f"Unsupported fake-MX quant targets {sorted(unknown_targets)}; "
                f"expected a subset of {sorted(SUPPORTED_QUANT_TARGETS)}."
            )
        self.quant_targets = frozenset(targets)
...
class _AscendFakeMXFusedMoEMethod(AscendMoEScheme):
    ...
    def __init__(self):
        ...
        self.rht_matrix_size = int(
            quant_description.get("rht_matrix_size", DEFAULT_TRANSFORM_MATRIX_SIZE)
        )
        self.rht_params_path = quant_description.get("rht_params_path")
        if self.algorithm == "rht" and not self.rht_params_path:
            raise ValueError("RHT requires rht_params_path.")
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if self.algorithm == "rht":
            params = _load_transform_params(self.rht_params_path)
            signs = torch.empty(self.rht_matrix_size, device=layer.w13_weight.device, dtype=torch.int8)
            key = _copy_rht_signs(signs, params, layer)
            layer.rht_signs = torch.nn.Parameter(signs.contiguous(), requires_grad=False)
            logger.debug("RHT MoE: loaded %s", key)
    ...
    def apply(
        ...
        if self.algorithm == "rht":
            x = randomized_hadamard_transform(x, layer.rht_signs, self.rht_matrix_size)
        ...
        return moe_comm_method.fused_experts(
            fused_experts_input=build_fused_experts_input(
                ...
                fake_mx_rht_signs=getattr(layer, "rht_signs", None),
