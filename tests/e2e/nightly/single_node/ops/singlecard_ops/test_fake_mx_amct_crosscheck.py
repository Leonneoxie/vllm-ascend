...
def test_amct_vllm_rht_signs_bit_identical():
    ...
    exported_signs = exported["rht_signs"].detach().cpu()
    assert exported_signs.dtype == torch.int8
    assert exported_signs.shape == (RHT_MATRIX_SIZE,)
    assert torch.all((exported_signs == -1) | (exported_signs == 1))
...
def _build_linear_method(method_cls, dtype):
    ...
    for key, value in method.get_weight(K_FEATURES, 256, dtype).items():
...
def test_amct_vllm_lht_matches(dtype):
    ...
    Both sides directly multiply Q in the operand dtype, without inversion.
    """
    ...
    with ExitStack() as stack:
        ...
        method, layer = _build_linear_method(LHTLinearMethod, dtype)
...
def test_amct_vllm_omniquant_matches(dtype):
    ...
    with ExitStack() as stack:
        for cm in _linear_method_patches(config, {"layer.log_scale": log_scale}):
            stack.enter_context(cm)
        method, layer = _build_linear_method(OmniQuantLinearMethod, dtype)
        layer.weight.data.copy_(W)
        method.prepare_weight(layer)
        ours_act = method.transform_activation(layer, x)
        ours_w = layer.weight.data
