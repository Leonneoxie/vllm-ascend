...
from vllm_ascend.quantization.methods.fake_mx_algorithms import common, flatquant, lht, linear, moe, omniquant, rht


@pytest.mark.parametrize("dtype", [torch.float32, torch.float16, torch.bfloat16])
def test_lht_sidecar_accepts_rounded_orthogonal_matrix(dtype):
    generator = torch.Generator().manual_seed(42)
    matrix, _ = torch.linalg.qr(torch.randn(128, 128, generator=generator))
    lht.validate_lht_matrix(matrix.to(dtype), name="test.transform_weight")


@pytest.mark.parametrize("value", [2.0, float("nan"), float("inf")])
def test_lht_sidecar_rejects_invalid_matrix(value):
    matrix = torch.eye(128)
    matrix[0, 0] = value
    with pytest.raises(ValueError, match="non-orthogonal transform"):
        lht.validate_lht_matrix(matrix, name="test.transform_weight")


def test_lht_linear_rejects_sidecar_before_weight_rotation():
    method = object.__new__(lht.LHTLinearMethod)
    method.matrix_size = 2
    method.params_path = "legacy.safetensors"
    ...
    with (
        patch.object(lht, "_load_transform_params", return_value={"layer.transform_weight": torch.eye(2) * 2}),
        pytest.raises(ValueError, match="legacy.safetensors:layer.transform_weight"),
    ):
