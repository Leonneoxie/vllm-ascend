...
# This file is a part of the vllm-ascend project.
#
"""Per-expert fake-MX activation transform math tests.

The transforms live in vllm_ascend.quantization.methods.fake_mx_algorithms
.expert_transforms; the grouping primitives they build on (cumsum_group_list)
stay in ops and are imported from there.
"""
...
class TestDispatchSelection(unittest.TestCase):
    def test_hadamard_learning_selects_transform_after_expert_dispatch(self):
        ...
        with patch(
            "vllm_ascend.quantization.methods.fake_mx_algorithms.expert_transforms.torch_npu"
            ".npu_grouped_matmul",
            side_effect=_fake_grouped_matmul,
            create=True,
        ):
